...
public class EditincomeFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_editincome,container,false);

        moneydb = new DatabaseHelper(getActivity());

        String value = getArguments().getString("Key2");
        idRow = (EditText) view.findViewById(R.id.idRow);
        idRow.setText(value);

        Cursor res = moneydb.getIncomeDataById(Integer.parseInt(value));
        res.moveToFirst();
        ...
        deletedata.setOnClickListener(
                new View.OnClickListener(){
                    ...
                    public void onClick(View v) {
                        Integer deleteRow = moneydb.deleteIncomeData(idRow.getText().toString());
                        if (deleteRow > 0)
